...
##### Краткая описание логики работы проекта
...
//Класс/метод Контролллера  испольлуют методы из моделей классов, в прмере подключается файл с классом App\Models\Users.php и выполняет метод выполнения запроса из базы данных польщзователя с 'id' = 1, метод в свою очередь обращается к классу Database\QueryBuilder построитель запросов:
["model"]=>
  object(App\Models\Users)#5 (1) {
    ["db"]=>
    object(Database\QueryBuilder)#6 (1) {
...
//Осуществляется создание объекта вида с помощью класса App\Core\View.  
["view"]=>
  object(App\Core\View)#4 (3) {
    ["pach"]=>
...
##### Краткая инструкция для разворачивания проекта у себя
